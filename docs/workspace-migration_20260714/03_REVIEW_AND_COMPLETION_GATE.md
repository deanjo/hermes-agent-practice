...
1. `SOURCE_COMPLETE`：代码、测试、提交和独立 review 齐全；例如 Kit 75/75 本地测试通过，只能证明这一层。
...
- Product 的最终提交只来自 Kit 项目；Product/mention 不新增 Hermes core hunk。允许原子替换 Kit 自有插件目录；既有 legacy compat 单独列账，仍不得整文件带入旧 gateway 三文件。
- T5v 默认只有 Guardrails 项目改动；若要改 Hermes 核心，必须先有失败测试证明公开 hook 无法表达首次停止。
